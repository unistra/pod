...
ADD_OVERVIEW_CMD = ENCODE_OVER_SSH_CMD + " '" + "for i in $(seq 0 100); do ffmpegthumbnailer -t $i%% -s 114x64 -c jpeg -i \"%(src)s\" -o %(out)s_strip$i.jpg; montage -geometry +0+0 %(out)s %(out)s_strip$i.jpg %(out)s; done; rm %(out)s_strip*.jpg" + "'"
