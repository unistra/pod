...
ADD_OVERVIEW_CMD = ENCODE_OVER_SSH_CMD + " '" + "for i in $(seq 0 99); do ffmpegthumbnailer -t $i%% -s %(scale)s -c jpeg -i \"%(src)s\" -o %(out)s_strip$i.jpg; montage -geometry +0+0 %(out)s %(out)s_strip$i.jpg %(out)s; done; rm %(out)s_strip*.jpg" + "'"
